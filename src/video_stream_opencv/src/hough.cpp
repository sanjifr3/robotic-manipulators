...
class HoughDetecter
{
  ...
  public:
    HoughDetecter()
      : it_(nh_)
    {
      image_sub_ = it_.subscribe("/camera/image", 1, &HoughDetecter::imageCb, this);
      //image_sub_ = it_.subscribe("/webcam/image_raw", 1, &HoughDetecter::imageCb, this);
      image_pub_ = it_.advertise("/camera/image_processed", 1);
      
      pub_c1 = n.advertise<geometry_msgs::Pose2D>("/block1_coord", 1, true);
      pub_c2 = n.advertise<geometry_msgs::Pose2D>("/block2_coord", 1, true);
      pub_c3 = n.advertise<geometry_msgs::Pose2D>("/block3_coord", 1, true);
      pub_c4 = n.advertise<geometry_msgs::Pose2D>("/block4_coord", 1, true);
      pub_c5 = n.advertise<geometry_msgs::Pose2D>("/block5_coord", 1, true);
    }
      ...
      cv::inRange(image_hsv, cv::Scalar(0, 100, 90), cv::Scalar(10, 255, 255), image_low_threshold);
      cv::inRange(image_hsv, cv::Scalar(160, 100, 90), cv::Scalar(179, 255, 255), image_high_threshold);
      ...
      cv::HoughCircles(image_red, circles, CV_HOUGH_GRADIENT, 1, 5, 100, 20, 0, 0);
      ...
      for(size_t i = 0; i < circles.size(); i++)
      {
          ...
          //sort each center(x,y) value into IJ matrix.
          IJ[i][0] = center.x;
          IJ[i][1] = center.y;

          //show how many circles are detected
          cnt++;
